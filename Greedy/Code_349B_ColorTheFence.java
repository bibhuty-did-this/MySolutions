...
public class Code_349B_ColorTheFence{

	static class ProblemSolver{
		public void solveTheProblem(InputReader in,OutputWriter out){
			int v=in.nextInt();
			ArrayList<Capacity> capacities=new ArrayList<>();
			for(int i=1;i<=9;++i)capacities.add(new Capacity(i,in.nextInt()));
			Collections.sort(capacities,(o1,o2)->o1.cap==o2.cap?Integer.compare(o1.num,o2.num):Integer.compare(o1.cap,o2.cap));
			int minCap=capacities.get(0).cap,minNum=-1;
			for(int i=0;i<9;++i){
				if(capacities.get(i).cap==minCap){
					minNum=Math.max(minNum,capacities.get(i).num);
				}
			}
			if(minCap>v){
				out.print(-1);
			...
			int length=v/minCap;
			v-=length*minCap;
			Collections.sort(capacities,Comparator.comparingInt(o->o.num));
			for(int i=0;i<length;++i){
				int num=minNum;
				for(int j=8;j>=0;--j){
					if(capacities.get(j).cap-minCap<=v && capacities.get(j).num>minNum){
						v-=(capacities.get(j).cap-minCap);
						num=capacities.get(j).num;
						break;
					}
				}
				out.print(num);
	...
	static class Capacity{
		int num,cap;
		public Capacity(int num,int cap){
			this.num=num;
			this.cap=cap;
		}
	}
