...
public class Code_616B_DinnerWithEmma{

    static class ProblemSolver{
        public void solveTheProblem(InputReader in,OutputWriter out){
            int n=in.nextInt(),m=in.nextInt();
            int[][] c=in.nextIntMatrix(n,m);
            ArrayList<Integer> cost=new ArrayList<>();
            int max=Integer.MIN_VALUE;
            for(int i=0,min=Integer.MAX_VALUE;i<n;cost.add(min),max=Math.max(max,min),min=Integer.MAX_VALUE,++i)
                for(int j=0;j<m;min=Math.min(min,c[i][j]),++j);
            out.print(max);
